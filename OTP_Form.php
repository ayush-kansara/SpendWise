...
<head>
  ...
  <title>OTP Verification - SpendWise</title>
...
<body>
  ...
  <main>
    <div class="login-form-container" id="otp-container">
      <p id="login-heading">ENTER OTP</p>
      <p id="sub-heading">Please check your email: <?php echo $_SESSION["email"] ?></p>
      <?php
      if (isset($_GET["error"])) { ?>
        <p style="color: red; text-align:center;"><?php echo $_GET["error"]; ?></p>
      <?php
      } ?>
      <form method="post" id="form" action="OTP_Verify.php" autocomplete="off">
        <div class="form-element-container">
          <div id="fields">
            <label for="uname" style="margin-left: 5px"><b>Enter OTP</b></label>
            <input
              type="text"
          ...
          <button type="submit" id="register-btn" name="otpBtn">Submit</button>
        </div>
      </form>
    </div>
  </main>
  ...
  <footer>
    <div class="footerContainer" id="otp">
  ...
  <script>
    ...
    window.onload = function() {
      setTimeout(function() {
        alert("OTP sent on your email: <?php echo $_SESSION["email"]; ?>");
        otpField.focus();
      }, 500);
